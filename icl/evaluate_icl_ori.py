...
import transformers
import sys
sys.path.append("/home/liupeiyu/AutoCompressors/models")
...
def read_args():
    ...
    parser.add_argument("--share_module",type=str,default="")
...
def get_model_tokenizer_device_isac(args):
    ...
    if "autocompressor-llama" in args.model_path.lower(): # LLaMA-2 AC
        ...
        is_ac = False
    if args.share_module:
        share_weights = None
        share_module = args.share_module
        print(f"Using weights of sharing: {share_module}, before sharing:{get_parameter_number(model)}")
        
        num_layers = 16
        for name,param in list(model.named_parameters()):
            if share_module in name:
                ind = re.findall(r"\d+",name)[0]
                if int(ind) >= num_layers:
                    continue
                if share_weights is None:
                    share_weights = copy.deepcopy(param)
                    share_weights.data = share_weights.data / num_layers
                else:
                    share_weights.data += param.data / num_layers
                
                name = name.replace(f".{ind}",f"[{ind}]")
                obj_name, weight_name = name.rsplit('.',1)
                print(f"sharing param: {obj_name}")
                setattr(eval("model."+obj_name),weight_name,share_weights)
...
class PromptGenerator(torch.utils.data.Dataset):
    ...
    def get_calibration_nlls(self, example: dict, model, device, is_ac: bool, softprompt=None, plaintext_demonstrations_tokens=None):
        ...
        for label_idx in range(len(example["options"])):
            ...
            with torch.no_grad():
                calibration_option_logits = model.forward(plaintext_tokens, use_cache=False)["logits"][:,-option_length-1:-1,:]
