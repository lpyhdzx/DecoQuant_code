...
class MPO:
    ...
    def get_tensor_set(self, inp_matrix):
        ...
        self.index_permute = torch.transpose(
            torch.arange(len(self.mpo_input_shape) + len(self.mpo_output_shape)).reshape((2, -1)), 0, 1).flatten()
        res = res.permute(*self.index_permute)
        #################################################################################
        for i in range(self.num_dim - 1):
            # Do the SVD operator
            res = res.reshape([self.mpo_ranks[i] * self.mpo_input_shape[i] * self.mpo_output_shape[i], -1])
            u, lamda, v = torch.linalg.svd(res,full_matrices=False)
            ...
            tensor_set.append(u)
            res = torch.mm(torch.diag(lamda), v)
    ...
    def left_canonical(self,tensor_set):
        ...
        u, lamda, v = torch.linalg.svd(mat,full_matrices=False)
        left_canonical_tensor[1] = torch.matmul(torch.diag(lamda), v)
        for i in range(1,self.num_dim-1):
            mat = torch.tensordot(left_canonical_tensor[i], tensor_set[i],dims=([1],[0]))
            mat = mat.reshape(-1, mat.shape[-1])
            u,lamda,v = torch.linalg.svd(mat,full_matrices=False)
            left_canonical_tensor[i+1] = torch.matmul(torch.diag(lamda), v)
    ...
    def right_canonical(self, tensor_set):
        ...
        u, lamda, v = torch.linalg.svd(mat,full_matrices=False)
        right_canonical_tensor[self.num_dim - 1] = torch.matmul(u, torch.diag(lamda))

        for i in range(self.num_dim - 2, 0, -1):
            mat = torch.tensordot(tensor_set[i], right_canonical_tensor[i + 1], dims=([3], [0]))
            mat = mat.reshape(mat.shape[0], -1)
            u, lamda, v = torch.linalg.svd(mat,full_matrices=False)
            right_canonical_tensor[i] = torch.matmul(u, torch.diag(lamda))
    ...
    def expectrum_normalization(self, lamda):
        ...
        :return:
        """
        norm_para = torch.sum(lamda ** 2) ** (0.5)
        lamda_n = lamda / norm_para
        lamda_12 = lamda ** (-0.5)
        return lamda_n, torch.diag(lamda_12)
    ...
    def gauge_aux_p_q(self, left_canonical_tensor, right_canonical_tensor):
        ...
        lamda_set[0] = torch.ones([1, 1])
        lamda_set[-1] = torch.ones([1, 1])
        
        for i in range(1, self.num_dim):
            mat = torch.matmul(left_canonical_tensor[i], right_canonical_tensor[i])
            u, lamda, v = torch.linalg.svd(mat,full_matrices=False)
            lamda_n, lamda_l2 = self.expectrum_normalization(lamda)
            lamda_set[i] = lamda_n
            lamda_set_value[i] = lamda
            p[i] = torch.matmul(right_canonical_tensor[i], v.t())
            p[i] = torch.matmul(p[i], lamda_l2)
            q[i] = torch.matmul(lamda_l2, u.t())
            q[i] = torch.matmul(q[i], left_canonical_tensor[i])
        
        return p, q, lamda_set, lamda_set_value

    def mpo_canonical(self, tensor_set, p, q):
        tensor_set[0] = torch.tensordot(tensor_set[0], p[1], dims=([3], [0]))
        tensor_set[-1] = torch.tensordot(q[self.num_dim-1], tensor_set[-1], dims=([1], [0]))
        
        for i in range(1, self.num_dim-1):
            tensor_set[i] = torch.tensordot(q[i], tensor_set[i], dims=([1], [0]))
            tensor_set[i] = torch.tensordot(tensor_set[i], p[i+1], dims=([3], [0]))
