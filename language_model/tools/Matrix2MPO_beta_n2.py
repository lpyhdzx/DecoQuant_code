...
np.random.seed(seed)
import sys
sys.path.append("/home/liupeiyu/flan-eval/mpo_c/build")
# sys.path.append("/home/liupeiyu/flan-eval/mpo_c_2/build/lib.linux-x86_64-cpython-38")
sys.path.append("/home/liupeiyu/flan-eval/bdcsvd/build/lib.linux-x86_64-cpython-38")
import svd_module

def randomized_svd(matrix, k):
    """
    Computes an approximate SVD for a matrix.
    :param matrix: Input matrix
    :param k: Number of singular values and vectors
    :return: Approximate U, S, Vt matrices
    """
    # Randomly generate an orthogonal matrix
    random_matrix = np.random.randn(matrix.shape[1], k)
    projected_matrix = np.dot(matrix, random_matrix)
    Q, _ = np.linalg.qr(projected_matrix)
    
    smaller_matrix = np.dot(Q.T, matrix)
    U_hat, S, Vt = np.linalg.svd(smaller_matrix, full_matrices=False)
    U = np.dot(Q, U_hat)
...
def truncated_svd(matrix, k):
    """
    Computes truncated SVD for a matrix.
    :param matrix: Input matrix
    :param k: Number of singular values and vectors
...
class MPO:
    ...
    def get_tensor_set(self, inp_matrix):
        ...
        for i in range(self.num_dim - 1):
            ...
            # u, lamda, v = np.linalg.svd(res, full_matrices=False) # randomized_svd
            #### 近似svd
    ...
    def gauge_aux_p_q(self, left_canonical_tensor, right_canonical_tensor):
        ...
        for i in range(1, self.num_dim):
            ...
            #garbage**** mat = right_canonical_tensor[i]
    ...
    def matrix2mpo(self, inp_matrix, cutoff=True):
        ...
        # tensor_set_obj = get_tensor_set.TensorSet(self.mpo_input_shape, self.mpo_output_shape, self.mpo_ranks)
        # tensor_set = tensor_set_obj.get_tensor_set(inp_matrix)
    ...
    def bi_canonical(self, tensor_set):
        ...
        return tensor_set
    def mpo2matrix_n2(self, tensor_set, new_shape):
        """
        ...
        t = torch.tensordot(tensor_set[0], tensor_set[1], ([3],[0]))
        # Squeeze the first and the last 1 dimension
        t = t.squeeze(0)
        t = t.squeeze(-1)
        # Caculate the new index for mpo
        # tmp1 = torch.tensor(range(len(self.mpo_output_shape))) * 2 # [0,2]
        # tmp2 = tmp1 + 1
        # new_index = torch.cat((tmp1, tmp2), 0)
        new_index = [0,2,1,3]
        # Transpose and reshape to output
        t = t.permute(tuple(new_index)).reshape(new_shape)
