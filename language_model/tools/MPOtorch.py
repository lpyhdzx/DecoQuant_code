...
class LinearDecomMPO(nn.Module):
    ...
    def __init__(self, mpo_input_shape, mpo_output_shape, trunc_num,
        ...
        **kwargs
        ...
        self.bias = None
        # from nn.Linear
        self.in_features = np.product(mpo_input_shape)
        self.out_features = np.product(mpo_output_shape)
    ...
    def forward(self, x):
        ...
        if self.lora_linear:
            ...
            res = F.linear(res, mpo.mpo2matrix(self.tensor_set).to(torch.float16),self.bias) # TODO
        ##################### use rebuild
        ori_shape=x.shape
        ...
        # return res
    def from_pretrained(self, input_shape,kernel_pretrain,tensor_set,dtype,bias=None,use_bias=True, device=None):
        if device:
            self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i).to(device)) for i in tensor_set])
            # if dtype:
            #     self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i).to(dtype).to(device)) for i in tensor_set])
            # else:
            #     self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i).to(device)) for i in tensor_set])
        else:
            self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i)) for i in tensor_set])
            # if dtype:
            #     self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i).to(dtype)) for i in tensor_set])
            # else:
            #     self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i)) for i in tensor_set])
        ...
        if self.tensor_learn:
            ...
            if dtype == torch.int8:
                # self.tensor_set[CT_index] = replace_8bit_linear(self.tensor_set[CT_index])
                raise NotImplementedError
            else:
                self.tensor_set[CT_index] = self.tensor_set[CT_index].to(torch.float32) # 固定权重的type # eval 时必须要改成这个
            # self.tensor_set[5].requires_grad = False
            for ind in [0,1,3,4]:
                self.tensor_set[ind] = self.tensor_set[ind].to(torch.float32)   # 训练权重的type
            self.tensor_set[CT_index].requires_grad = False
            self.tensor_set[0].requires_grad = True
            self.tensor_set[1].requires_grad = True
            self.tensor_set[3].requires_grad = True
            self.tensor_set[4].requires_grad = True
        elif self.CT_learn:
        ...
        if use_bias and bias:
            self.bias = bias.to(device) if device else bias
        else:
